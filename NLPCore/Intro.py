...
# sent_tokenize method splits text by sentences(different sentences[non repeating]),
sentences = nltk.sent_tokenize(paragraph)
print(len(sentences)) # 21 sentences

# word_tokenize methods splits text by words(different words)
words = nltk.word_tokenize(paragraph)
print(len(words)) #   346 words

# stemming and lemmatization
# stemming - reducing infected or derived words to their word stem, base or root form
# intelligent, intelligence, intelligently = inteligen
# gone, goes, going = go
# final, finally, finalize = fina

# problem: intermediate words that come from stemming may not have any meaning such as the above
# solution: lemmatization - same as stemming but intermediate words have meaning
# EX: the above stemming examples would become, (intelligent, go, and final) takes more time than stemming
# NOTE: use stemming when meaning of word is not important, EX: spam detection
# chat bots work better with lemmatized words, but research shows that we should do either
# instead processing meaning seperately is at times a better solution, howeve, there are useful for
# when processing bodies of text
